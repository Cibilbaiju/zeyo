...
class _ServiceDetailScreenState extends State<ServiceDetailScreen> {
  ...
  Future<void> _handleBookService(Map<String, dynamic> service) async {
    ...
    try {
      ...
      // Mock Lat/Lng for now (User's location)
      // Ideally get from LocationService (if we add it to User App)
      const double pickupLat = 12.9716; 
      const double pickupLng = 77.5946;

      final response = await ApiService.post('/jobs/request', {
        'userId': user.id, // Assuming 'id' is available in User model
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Booking Requested! Job ID: ${response['id']}')),
        );
        // Maybe navigate to a "Scanning/Matching" screen
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
                                     ...
                                     Row(
                                       ...
                                       children: [
                                          Text(
                                           ...
                                           TextButton(
                                            onPressed: _isBooking ? null : () => _handleBookService(variant), 
